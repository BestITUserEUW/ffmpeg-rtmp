...
int main() {
    ...
    int ret;
    if ((ret = av_dict_set(&options, "rtmp_live", "live", 0)) < 0) {
        println("Failed to set listen mode for server");
        return ret;
    }

    ret = avformat_alloc_output_context2(&fmt_ctx, nullptr, "flv", nullptr);
    if (ret < 0) {
    ...
    video_stream = avformat_new_stream(fmt_ctx, codec_ctx->codec);
    ...
    ret = avio_open2(&fmt_ctx->pb, url, AVIO_FLAG_WRITE, nullptr, &options);
    if (ret < 0) {
